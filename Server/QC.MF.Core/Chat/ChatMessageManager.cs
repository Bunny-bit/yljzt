...
{
    [AbpAuthorize]
    public class ChatMessageManager : MFDomainServiceBase, IChatMessageManager
    {
        ...
        private void TryToSendChatMessageMail(User user, string userName, string senderTenancyName, ChatMessage sentMessage)
        {
            try
            {
                _emailSender.Send(user.EmailAddress, "新消息提醒", $"发送人：{userName}<br />时间：{sentMessage.CreationTime.ToString("yyyy-MM-dd HH:mm:ss")}<br />内容：{sentMessage.Message}<br />");
            }
