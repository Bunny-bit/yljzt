...
{
    public static string Format(string originFileName, string pathFormat,out string  error)
    {
        error = "";
        ...
        var userId = (HttpContext.Current?.User as ClaimsPrincipal)?.Claims.FirstOrDefault(x => x.Type == ClaimTypes.NameIdentifier)?.Value;
        if (string.IsNullOrEmpty(userId))
        {
            error = "用户没有登录。";
        }
        pathFormat = pathFormat.Replace("{userid}", userId);
